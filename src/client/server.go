...
func (root *Takina) Run() {
	...
	server.AddRouter(types.ROUTER_TAKINA_CLIENT_DAEMON_START_PROXY, &TakinaClientDeamonRequestStartProxy{})
	server.AddRouter(types.ROUTER_TAKINA_CLIENT_DAEMON_STOP_PROXY, &TakinaClientDeamonRequestStopProxy{})
	server.AddRouter(types.ROUTER_TAKINA_CLIENT_DAEMON_LIST_PROXY, &TakinaClientDeamonRequestListProxy{})
	server.Serve()
}
